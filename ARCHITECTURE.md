...
- `app/page.tsx` — Main page (`'use client'`). State: `previewColor`, `sheetOpen`, `pickedColor`. Renders full empty state: hero 12-col grid (eyebrow, H1, swatch trigger, hex display, CTA); right panel (how it works, status dot); 8-tile suggested swatches grid with hover preview; footer bar. Swatch trigger, CTA, and preset swatches all wire to `setSheetOpen(true)`. Preset click also commits `setPickedColor`. Hero swatch and hex display update to reflect `pickedColor` (committed) vs `previewColor` (hover) vs empty. Renders `<ColorPickerSheet>` at page root.
